...
string problemA3Solution(string &originalStr, int &vowelCount, int &consonantCount)
{
    ...
    for (size_t i = 0; i < originalStr.size(); i++)
    {
        ...
        {
            result.push_back(toupper(originalStr[i]));
        }
        originalStr[i] = toupper(originalStr[i]);
    }
    char vowels[]{"AEUIO"};
    for (size_t i = 0; i < result.size(); i++)
    {
        if (strchr(vowels, result[i]) != nullptr)
        {
            vowelCount++;
        }
    }
    consonantCount = result.size() - vowelCount;
                      ...
                      int **negativeArr, int &negSize)
{
    ...
    unsigned int negArr = negSize;
    unsigned int posArr = posSize;
    if (negSize == 0)
    {
        *negativeArr = nullptr;
    }
    ...
    for (int i = 0; i < size; i++)
    {
        if (arr[i] < 0)
        {
            (*negativeArr)[negSize - negArr] = arr[i];
            negArr--;
        }
        else if (arr[i] > 0)
        {
            (*positiveArr)[posSize - posArr] = arr[i];
            posArr--;
        }
    }
...
void knowledgeQuiz()
{
    ...
    cout << "Correct Answer: B - Output parameters allow functions to return multiple values "
         << "by passing references or pointers that the function modifies." << endl
         << endl;
    ...
    cout << "Correct Answer: D - Both pointers (int* x) and references (int& x) can be used "
         << "as output parameters. References are generally preferred for cleaner syntax." << endl
         << endl;
    ...
    cout << "Correct Answer: A - Return by value creates a copy of the local variable. "
         << "The original local variable is destroyed when the function exits." << endl
         << endl;
    ...
    cout << "Correct Answer: B - This returns a pointer to a local array that will be destroyed "
         << "after the function ends, creating a dangling pointer. Use dynamic allocation instead." << endl
         << endl;
    ...
    cout << "Correct Answer: A - Return by value is cleaner and more intuitive for single values. "
         << "Use output parameters when you need to return multiple values." << endl
         << endl;
    ...
    cout << "Correct Answer: B - References cannot be null and must be initialized, "
         << "providing compile-time safety. They also have cleaner syntax than pointers." << endl
         << endl;
    ...
    cout << "Correct Answer: B - 'const int&' is an input parameter passed by reference "
         << "for efficiency but cannot be modified. This is NOT an output parameter." << endl
         << endl;
    ...
    cout << "Correct Answer: B - Functions can return references (useful for operator overloading), "
         << "but never return a reference to a local variable as it will be destroyed." << endl
         << endl;
    ...
    cout << "Correct Answer: C - A return statement can only return one value. "
         << "Use output parameters, structs, or containers to return multiple values." << endl
         << endl;
    ...
    cout << "Correct Answer: B - Non-const references/pointers allow reading the input value "
         << "and modifying it to produce output. Example: void func(int& x) { x = x * 2; }" << endl
         << endl;
    ...
    cout << "Correct Answer: C - When a function allocates memory and returns it via pointers "
         << "(output parameters or return value), the caller is responsible for deallocating it." << endl
         << endl;
    ...
    cout << "Correct Answer: C - Output parameters don't need initialization before passing "
         << "because the function will assign values to them. However, they must be declared." << endl
         << endl;
    ...
    cout << "Correct Answer: B - Passing large objects through output parameters (references) "
         << "avoids copying. However, modern C++ with move semantics can optimize return by value." << endl
         << endl;
    ...
    cout << "Correct Answer: B - If the function doesn't properly assign to the output parameter, "
         << "the variable will retain its previous value or garbage. No compilation error occurs." << endl
         << endl;
    ...
    cout << "Correct Answer: B - Common pattern: return success/failure status while using "
         << "output parameters for actual data. Example: bool getData(int& result);" << endl
         << endl;

    cout << "========================================" << endl;
    cout << "QUIZ COMPLETE! Your score: " << score << "/15" << endl;
    if (score == 15)
        cout << "Perfect! You've mastered Chapter 12!" << endl;
    else if (score >= 12)
        cout << "Excellent! Strong understanding of the concepts." << endl;
    else if (score >= 9)
        cout << "Good! Review the missed concepts." << endl;
    else
        cout << "Keep studying! Review the chapter material." << endl;
    cout << "========================================" << endl;
}
